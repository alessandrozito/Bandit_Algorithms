...
def test_algorithm(algo, X_arms, X_hat, horizon, size_batch):
    ...
    best_arm = int(X_arms[X_arms['success_probability'] == \
    max(X_arms['success_probability'])]['arm'])
    best_prob = max(X_arms['success_probability'])


    # Initialize the quantities
    algo.initialize(size_batch)
    arms_responses = list(map(lambda p: Arm_response(p), \
    X_arms['success_probability']))
    probs = list(X_arms['success_probability'])

    if size_batch == 1:
        simulation_dict['chosen_arms'] = [0.0 for col in range(horizon)]
        simulation_dict['chosen_probs'] = [0.0 for col in range(horizon)]

    # Name the algorithm
    if algo.__class__.__name__ == 'ThompsonSUN_SMC':
        simulation_dict['algorithm']= [algo.name(horizon) for col in \
        range(horizon)]
    else:
        simulation_dict['algorithm'] = [algo.name() for col in range(horizon)]

    # Start the cycle
    for t in range(horizon):
        print(algo.__class__.__name__, t)
        day_time = time.time()
        t = t + 1
        index = t - 1
        ...
        if algo.__class__.__name__ not in  ['ThompsonSUN_SMC','ThompsonGibbs', \
        'ThompsonSUN_IndGamma'] :
            successes = np.array(list(map(lambda arm, size: \
            arms_responses[arm].sumdraw(size), list(X_hat.columns), chosen_arms)))
            reward = sum(successes)
            regret = sum(list(map(lambda x: chosen_arms[x] * \
            (best_prob - probs[x]), list(X_hat.columns))))
            ...
            for arm in range(len(X_hat.columns)):
                ...
                while pulls > 0:
                    ...
                    X_day = X_day.append(pd.Series(chosen_arm_features), \
                    ignore_index = True)
                    y_day = np.append(y_day, response)
                    chosen_prob = float(X_arms[X_arms['arm']\
        ...
        if t == 1:
            ...
            simulation_dict['cumulative_rewards'][index] = \
            simulation_dict['cumulative_rewards'][index-1] + reward
            simulation_dict['cumulative_regrets'][index] = \
            simulation_dict['cumulative_regrets'][index-1] + regret
        simulation_dict['completion_time'][index] = time.time() - day_time

        if size_batch == 1:
            simulation_dict['chosen_arms'][index] = chosen_arms.index(1)
            simulation_dict['chosen_probs'][index] = probs[chosen_arms.index(1)]
