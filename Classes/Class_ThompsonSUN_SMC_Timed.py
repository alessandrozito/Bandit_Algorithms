...
class ThompsonSUN_SMC():
    ...
    def __init__ (self, mu, Sigma, size_wat, time_cutoff):
    ...
    def select_arm(self, X_hat):
        ...
        if n == 0:
            ...
            absolute_weights = np.array(list(map(lambda beta: np.prod(norm.cdf((2*y_batch -1)*np.matmul(X_batch, beta))), self.theta)))
            importance_weights = absolute_weights/sum(absolute_weights)
            ''' Resample from the previous particles '''
            self.theta = random.choices(self.theta, weights=importance_weights, k=self.size_wat)
    ...
    def return_time_cutoff(self):
        return float(self.time_cutoff)
